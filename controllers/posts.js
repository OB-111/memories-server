...
export const getPostsBySeach = async (req, res) => {
  const { searchQuery, tags } = req.query;

  try {
    const title = new RegExp(searchQuery, "i");
    console.log(title, searchQuery, tags);
    const posts = await PostMessage.find({
...
export const updatePost = async (req, res) => {
  ...
  const updatedPoast = await PostMessage.findByIdAndUpdate(
    _id,
    { ...post, _id },
    {
      new: true,
